...
void main() {
  testWidgets('renders bars for tracks with data and a no-data marker for tracks without', (
    tester,
  ) async {
    final tracks = [
      _track(position: 1, name: 'Track One', danceability: 0.8, tempo: 120, loudness: -5.0),
      _track(position: 2, name: 'Track Two', danceability: 0.3, tempo: 140, loudness: -8.5),
      _track(position: 3, name: 'Track Three'), // no audio-feature data at all
    ];
  ...
  testWidgets('switching the dropdown to another field does not throw', (tester) async {
    final tracks = [
      _track(position: 1, name: 'Track One', danceability: 0.8, tempo: 120, loudness: -5.0),
      _track(position: 2, name: 'Track Two', danceability: 0.3, tempo: 140, loudness: -8.5),
    ];
  ...
  testWidgets('sorting ascending/descending does not throw with mixed null values', (
    tester,
  ) async {
    final tracks = [
  ...
  testWidgets('shows a message instead of a chart when no track has data', (tester) async {
    final tracks = [
    ...
    expect(find.text('No audio feature data for this mixlist yet.'), findsOneWidget);
  });
