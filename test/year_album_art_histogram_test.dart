...
Future<void> _pump(WidgetTester tester, Map<int, List<AlbumArtHistogramEntry>> entriesByYear) {
...
void main() {
  ...
  testWidgets('a year with zero entries still shows its count and year label', (tester) async {
  ...
  testWidgets('renders exactly one thumbnail per entry, across multiple years', (tester) async {
    await _pump(tester, {
  ...
  testWidgets('a year with more than 6 entries wraps into a new sub-column, column-major',
      (tester) async {
    ...
    expect(sixth.dx, first.dx, reason: 'entry 5 stays in the same sub-column as entry 0');
    expect(sixth.dy, greaterThan(first.dy), reason: 'entry 5 is below entry 0 within the sub-column');

    expect(seventh.dx, greaterThan(first.dx),
        reason: 'entry 6 starts a new sub-column to the right');
    expect(seventh.dy, first.dy,
        reason: 'entry 6 starts at the top of its sub-column, level with entry 0');
  });

  testWidgets('a year with exactly 12 entries produces two full 6-row sub-columns, no more',
      (tester) async {
  ...
  testWidgets(
    ...
    final loneEntryInShortColumn = tester.getTopLeft(find.byTooltip('entry-12'));

    expect(loneEntryInShortColumn.dy, lastRowOfFullColumn.dy,
        reason: 'the short sub-column\'s single entry sits at the bottom row, '
