...
The web build uses `sqflite_common_ffi_web`, which needs `web/sqlite3.wasm` and `web/sqflite_sw.js` present to open the database in the browser. These are generated, not checked into git, so run this once before building/running on web (and again whenever `flutter clean` removes them):

```
dart run sqflite_common_ffi_web:setup
```

Or use `tool/run_web.sh` instead of `flutter run -d chrome` — it generates the assets automatically if they're missing before launching.
